...
// Alvo padrão quando COPY_TARGET_WALLETS não está definido: perfil @coldmath
// (https://polymarket.com/@coldmath). Override via env COPY_TARGET_WALLETS.
const DEFAULT_TARGETS = ["0x594edb9112f526fa6a80b8f858a6379c8a2c1c11"];

/** Configuração pública (sem segredos) — pode ser devolvida em /status. */
export function getCopyConfig() {
  const fromEnv = String(process.env.COPY_TARGET_WALLETS || "")
    .split(",")
    .map((s) => s.trim().toLowerCase())
    .filter((s) => ADDR_RE.test(s));
  const targets = fromEnv.length > 0 ? fromEnv : [...DEFAULT_TARGETS];

  // "mirror" = cópia fiel 1:1 (padrão). Também aceita "fixed" e "proportional".
  const rawMode = process.env.COPY_SIZE_MODE;
  const sizeMode = ["fixed", "proportional", "mirror"].includes(rawMode) ? rawMode : "mirror";
  ...
  return {
    ...
    // Em modo fiel (mirror) o padrão é copiar TUDO (sem piso de valor de origem).
    minSourceUsd: num(process.env.COPY_MIN_SOURCE_USDC, sizeMode === "mirror" ? 0 : 50),
